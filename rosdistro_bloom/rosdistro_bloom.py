...
source_distribution_filename = index_yaml["distributions"][args.source]["distribution"][
    0
]
...
for repo_name, repo_data in sorted(source_distribution.repositories.items()):
    if repo_name not in dest_distribution.repositories:
        ...
        and dest_distribution.repositories[repo_name].release_repository.version is None
    ):
...
print(
    f"Found {len(repositories_to_retry)} repositories to retry:", repositories_to_retry
)
